...
import HanmacInfo from './components/Governance/HanmacInfo';
import OrdersSalesMain from './components/OrdersSales/OrdersSalesMain';
import MembersMain from './components/Members/MembersMain';
import CashflowMain from './components/Cashflow/CashflowMain';

// ─── detail-viewer 전용 컴포넌트 (별도 팝업 창에서 렌더링) ───
const DetailViewerWindow: React.FC = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100vw', height: '100dvh', background: '#F0EDE6', fontFamily: '"Noto Sans KR", sans-serif' }}>
      {/* 헤더 */}
      <div style={{
        height: 48, minHeight: 48, display: 'flex', alignItems: 'center', padding: '0 20px',
        background: 'linear-gradient(0deg, #07412e 0%, #29724F 80%, #37a184 100%)',
        borderBottom: '1px solid #135643', flexShrink: 0,
      }}>
        <span style={{ fontSize: 14, fontWeight: 700, color: '#fff', letterSpacing: '-0.3px' }}>
          {currentUrl ? '한맥가족 MIS 상세 정보' : '한맥가족 MIS 상세 정보 보기'}
        </span>
      </div>
      {/* 본문 */}
      <div style={{ flex: 1, minHeight: 0 }}>
        {currentUrl ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 16, color: '#737373' }}>
            ...
            <div style={{ fontSize: 20, fontWeight: 700, color: '#404040', letterSpacing: '-0.4px' }}>한맥가족 MIS 상세 정보 보기</div>
            <div style={{ fontSize: 13, color: '#A3A3A3', textAlign: 'center', lineHeight: 1.7, letterSpacing: '-0.2px' }}>
              메인 화면의 지배구조에서 회사를 클릭하거나<br />
              「가족사 규모」 버튼을 눌러 상세 정보를 확인하세요.
            </div>
          </div>
        )}
      </div>
...
const App: React.FC = () => {
  ...
  const [selectedGovNode, setSelectedGovNode] = useState<string | null>(null);
  ...
  const [dualPanelUrl, setDualPanelUrl] = useState<string | null>(null);
  ...
  useEffect(() => {
    const handleGovMessage = (e: MessageEvent) => {
      if (e.data?.type !== 'govDetailRequest') return;
      const nodeId: string = e.data.nodeId;
      const base = import.meta.env.BASE_URL ?? '/';
      const detailUrl = nodeId === 'scale'
        ? `${base}hm_misystem/상세페이지_지배구조_가족사규모.html`
        : `${base}hm_misystem/detail.html?id=${nodeId}`;
      if (childWindowRef.current && !childWindowRef.current.closed) {
        childWindowRef.current.postMessage({ type: 'showDetail', url: detailUrl }, '*');
      } else {
        openDetailViewer(detailUrl);
      }
    };
    window.addEventListener('message', handleGovMessage);
    return () => window.removeEventListener('message', handleGovMessage);
  ...
  const isDetailViewer = window.location.search.includes('view=detail-viewer');
  ...
  // Window Sync Logic: Sync selectedId and selectedGovNode across parent and child windows
  const selectedIdRef = useRef(selectedId);
  const lastBroadcastId = useRef<string | null>(null);
  const selectedGovNodeRef = useRef(selectedGovNode);
  const lastBroadcastGovNodeId = useRef<string | null>(null);
  ...
  useEffect(() => {
    selectedGovNodeRef.current = selectedGovNode;
  }, [selectedGovNode]);
  ...
  useEffect(() => {
    ...
    if (!isTimelineView) {
      ...
      channel.postMessage({ type: 'SELECT_GOV_NODE', id: null });
      lastBroadcastId.current = null;
      lastBroadcastGovNodeId.current = null;
    } else {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      } else if (event.data.type === 'SELECT_GOV_NODE') {
        const newGovNode = event.data.id;
        if (selectedGovNodeRef.current !== newGovNode) {
          lastBroadcastGovNodeId.current = newGovNode;
          setSelectedGovNode(newGovNode);
        }
      } else if (event.data.type === 'REQUEST_SYNC') {
        if (!isTimelineView) {
          channel.postMessage({ type: 'SELECT_PROJECT', id: selectedIdRef.current });
          channel.postMessage({ type: 'SELECT_GOV_NODE', id: selectedGovNodeRef.current });
        }
  ...
  useEffect(() => {
    if (selectedGovNode !== lastBroadcastGovNodeId.current) {
      lastBroadcastGovNodeId.current = selectedGovNode;
      const channel = new BroadcastChannel('dashboard_sync');
      channel.postMessage({ type: 'SELECT_GOV_NODE', id: selectedGovNode });
      channel.close();
    }
  }, [selectedGovNode]);
  ...
  const openDetailViewer = async (detailUrl?: string) => {
    let targetLeft = window.screenX + window.outerWidth;
    let targetTop = window.screenY;
    let targetWidth = window.screen.availWidth;
    let targetHeight = window.screen.availHeight;
    ...
    try {
      if ('getScreenDetails' in window) {
        ...
        const currentScreen = screenDetails.currentScreen;
        let nextScreen = screenDetails.screens.find((s: any) => s.left >= currentScreen.left + currentScreen.width);
        if (!nextScreen) nextScreen = screenDetails.screens.find((s: any) => s !== currentScreen);
        if (nextScreen) {
          targetLeft = nextScreen.availLeft ?? nextScreen.left;
          targetTop = nextScreen.availTop ?? nextScreen.top;
          targetWidth = nextScreen.availWidth ?? nextScreen.width;
          ...
          targetLeft = window.screenX + window.outerWidth;
          targetWidth = window.screen.availWidth - targetLeft;
          if (targetWidth < 800) { targetWidth = 1920; targetLeft = window.screen.availWidth; }
          targetHeight = window.screen.availHeight;
        }
      }
    } catch (err) {
      console.warn('Window Management API failed, using fallback', err);
    }

    const base = window.location.href.split('?')[0];
    const viewerUrl = detailUrl
      ? `${base}?view=detail-viewer&detail=${encodeURIComponent(detailUrl)}`
      : `${base}?view=detail-viewer`;
    const features = `left=${targetLeft},top=${targetTop},width=${targetWidth},height=${targetHeight},menubar=no,toolbar=no,location=no,status=no`;
    setIsDualMode(true);
    childWindowRef.current = window.open(viewerUrl, 'HM_Detail_Viewer', features);
  };



  // ─── detail-viewer 모드: 별도 창에서 상세 페이지를 전체화면으로 표시 ───
  if (isDetailViewer) {
    return <DetailViewerWindow />;
  }

  return (
    <div className={`app ${activeSummaryFilter === '보류' ? 'is-orders-sales-view' : ''} ${(selectedId && isTimelineView) || isWideScreen ? 'detail-docked' : ''} ${isTimelineView ? 'timeline-only-view' : ''} ${isDualMode && !isTimelineView ? 'dual-mode-parent' : ''}`}>
      <OrgChartPopup
        open={orgChartOpen}
      ...
      <section className="main">
        ...
        {activeSummaryFilter === '진행' && (
          <MembersMain teamMembers={teamMembers} projects={projects} />
        )}

        {activeSummaryFilter === '완료' && (
          <CashflowMain projects={projects} />
        )}
      ...
      {activeSummaryFilter !== '보류' && (
        ...
        {false && (isTimelineView || isWideScreen) && !selectedId && (
          selectedGovNode === 'hanmac' ? (
            <div className="detail-view gov-node-detail-view">
              <div className="detail-top">
                <div className="detail-top-left">
                  <div className="side-left-group">
                    <div className="side-title-container">
                      <div className="side-title-wrap">
                        <button className="back-btn" onClick={() => setSelectedGovNode(null)} title="뒤로가기">
                          <ChevronLeft size={20} />
                        </button>
                        <div className="detail-title-info">
                          <div className="detail-main-title">
                            <span className="side-title-main">한맥기술 정보</span>
                          </div>
                        </div>
                      </div>
                    </div>
                ...
                <div className="detail-top-right">
                  <button className="detail-close-btn" onClick={() => setSelectedGovNode(null)} title="닫기">
                    <X size={24} />
                  </button>
                </div>
              </div>
              <div className="detail-body" style={{ background: 'var(--bg-beige)', padding: '24px', overflowY: 'auto' }}>
                <HanmacInfo />
              </div>
            </div>
          ) : (
        ...
        {selectedId && false && (isTimelineView || isWideScreen) && (
