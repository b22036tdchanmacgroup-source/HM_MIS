...
const ShareholdersD3Chart: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // 컨테이너 그룹 추가 (차트를 아래로 약 28px 이동)
    const container = svg.append('g').attr('transform', 'translate(0, 28)');

    // 엣지
    const links = container.append('g').attr('fill', 'none')
