...
import { Monitor, Network, RotateCcw } from 'lucide-react';
...
// ─────────────────────────────────────────────
// 서브 컴포넌트: 좌측 — 타이틀 + 팀 현황 버튼
// ─────────────────────────────────────────────
const HeaderTitle: React.FC<{
  showTeamView: boolean;
  onToggle: () => void;
}> = ({ showTeamView, onToggle }) => (
  <div className="side-left-group">
    ...
    <button
      className={`team-status-btn-new ${showTeamView ? 'active' : ''}`}
      onClick={onToggle}
      title="주주현황 및 지배구조"
    >
      <Network size={16} />
...
const HeaderStats: React.FC<{
  activeSummaryFilter: string | null;
  setActiveSummaryFilter: React.Dispatch<React.SetStateAction<string | null>>;
}> = ({ activeSummaryFilter, setActiveSummaryFilter }) => (
    ...
    <div className="poly-stat-item" style={{ fontSize: '18px', gap: '8px' }}>
      {STAT_ITEMS.map((item) => (
        <React.Fragment key={item.filterKey}>
          <span
            onClick={() => {
              setActiveSummaryFilter(prev => prev === item.filterKey ? null : item.filterKey);
            }}
            className={`poly-click-stat ${activeSummaryFilter === item.filterKey ? 'active' : ''}`}
